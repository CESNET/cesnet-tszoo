...
import cesnet_tszoo.datasets.databases.abilene.constants as constants
from cesnet_tszoo.utils.enums import SourceType, AgreggationType, DatasetType


class Abilene(CesnetDatabase):
    """
    Dataset class for [Abilene][abilene]. 

    Use class method [`get_dataset`][cesnet_tszoo.datasets.abilene.get_dataset] to create a dataset instance.
    """
    name = "Abilene"
    bucket_url = "https://liberouter.org/datazoo/download?bucket=abilene"
    id_names = constants.ID_NAMES
    default_values = constants.DEFAULT_VALUES
    matrix_feature_mappings = constants.MATRIX_FEATURE_MAPPINGS
    ...
    @overload
    @classmethod
    def get_dataset(cls, data_root: str, subset: Literal["matrix", "node", "node2node"], aggregation: AgreggationType | Literal["5_minutes", "10_minutes", "1_hour", "1_day"], dataset_type: Literal[DatasetType.TIME_BASED, "time_based"], check_errors: bool = False, display_details: bool = False) -> TimeBasedCesnetDataset: ...

    @overload
    @classmethod
    def get_dataset(cls, data_root: str, subset: Literal["matrix", "node", "node2node"], aggregation: AgreggationType | Literal["5_minutes", "10_minutes", "1_hour", "1_day"], dataset_type: Literal[DatasetType.SERIES_BASED, "series_based"], check_errors: bool = False, display_details: bool = False) -> SeriesBasedCesnetDataset: ...

    @overload
    @classmethod
    def get_dataset(cls, data_root: str, subset: Literal["matrix", "node", "node2node"], aggregation: AgreggationType | Literal["5_minutes", "10_minutes", "1_hour", "1_day"], dataset_type: Literal[DatasetType.DISJOINT_TIME_BASED, "disjoint_time_based"], check_errors: bool = False, display_details: bool = False) -> DisjointTimeBasedCesnetDataset: ...

    @classmethod
    def get_dataset(cls, data_root: str, subset: Literal["matrix", "node", "node2node"], aggregation: AgreggationType | Literal["5_minutes", "10_minutes", "1_hour", "1_day"],
        ...
        return super(Abilene, cls).get_dataset(data_root, subset, SourceType.BACKBONE_NETWORK, aggregation, dataset_type, check_errors, display_details)
