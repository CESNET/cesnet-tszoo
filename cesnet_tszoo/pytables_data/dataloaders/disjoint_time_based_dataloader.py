...
class DisjointTimeBasedDataloader(DataLoader):
    ...
    def __init__(self, dataset: SplittedDataset, dataset_config: DisjointTimeBasedConfig, workers: int, take_all: bool, batch_size: int):
        ...
        if take_all:
            ...
            self.logger.debug("SplittedDataset prepared with full batch size (%d samples).", batch_size)
        else:
            ...
            self.logger.debug("SplittedDataset prepared with window size (%d).", dataset_config.sliding_window_size)
