...
class SeriesBasedDataset(BaseDataset):
    ...
    def __getitem__(self, batch_idx):
        fillers = self.fillers[batch_idx]
        ...
        for i, _ in enumerate(data):
            if len(self.indices_of_features_to_take_no_ids) == 1:
                data[i][:, self.indices_of_features_to_take_no_ids] = self.transformers.transform(data[i][:, self.indices_of_features_to_take_no_ids].reshape(-1, 1))
            elif len(self.time_period) == 1:
                data[i][:, self.indices_of_features_to_take_no_ids] = self.transformers.transform(data[i][:, self.indices_of_features_to_take_no_ids].reshape(1, -1))
            else:
                data[i][:, self.indices_of_features_to_take_no_ids] = self.transformers.transform(data[i][:, self.indices_of_features_to_take_no_ids])
