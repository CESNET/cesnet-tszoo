...
!!! info "Note for [`Time-based`](reference_time_based_cesnet_dataset.md#cesnet_tszoo.datasets.time_based_cesnet_dataset.TimeBasedCesnetDataset)"
...
### Built-in fillers
The `cesnet_tszoo` package comes with multiple built-in fillers. To check built-in fillers refer to [`fillers`](reference_fillers.md#cesnet_tszoo.utils.filler.filler).

### Custom fillers
It is possible to create and use own fillers. But custom filler must derive from [`Filler`](reference_fillers.md#cesnet_tszoo.utils.filler.filler.Filler) base class.